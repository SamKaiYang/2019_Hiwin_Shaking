...
class Kinematics_Model_cls():
    ...
    def KinematicsDraw(self,angle,clr_typ,line_width,perspective):
        ...
        # print('Node_{}_pos:'.format(6),np.around(node[6],decimals=1))
        if perspective == '3D':
...
class Simu_Ani_cls():
    ...
    def animate(self,i):
        ...
        if self.simu_mode:   #true for p2p
            ...
            #     print(theta,'(pre):',np.around(self.start_angle[theta],decimals=1))
            #     self.start_angle[theta]=ReformTheta(self.start_angle[theta]*pi/180,self.neg_lim_deg[theta],self.pos_lim_deg[theta])*180/pi
            #     print(theta,'(post):',np.around(self.start_angle[theta],decimals=1))
    ...
    def MySimuAni(self,begin_pos,end_pos,grip_angle,speed,simu_p2p):
        ...
        if num == 0:
            if self.grip_angle != grip_angle:
                num = int(abs(grip_angle-self.grip_angle)/(float(speed)**(2/3)))
                if num ==0:
                    num=1
            else:
                num = 1
        ...
        if self.simu_mode:   #true for p2p
            start_angle=self.Arm_Model.arm_center.InverseKinematics(self.start_pos)%720
            end_angle=self.Arm_Model.arm_center.InverseKinematics(end_pos)%720
            # for theta in range(6):
            #     start_angle[theta]=ReformTheta(theta,self.neg_lim_deg[theta],self.pos_lim_deg[theta])
            #     end_angle[theta]=ReformTheta(theta,self.neg_lim_deg[theta],self.pos_lim_deg[theta])
