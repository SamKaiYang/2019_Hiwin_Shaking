...
arm_speed_mode=0
move_mode=2 # 2 for p2p; 3 for line
speed_value=30
...
class Simulation_cls():
    ...
    def MoveSimulation(self,Position,speed,grip,string):
        ...
        self.Simu.MySimuAni(self.current_pos_array,PointToMatrix(Position),self.grip_angle,speed,self.p2p_mode)
...
class Shake_Strategy_cls():
    ...
    def SmartShaking(self,Action_cls):
        global step_flag,speed_value,delt_z,finished_flag,ice_end_tm,ice_start_tm,sugar_end_tm,sugar_start_tm
        def GetInFit():
            delt_l=0.3
